...
- [ ] Task: Setup and Initial Assessment
    - [ ] Run existing test suite and coverage report to establish baseline metrics.
    - [ ] Identify files in `@icarus_sentinel/` that exceed the 500-line limit.
    - [ ] Map out the existing monolithic structure and identify logical boundaries for separation (e.g., UI vs. Core Logic).
...
- [ ] Task: Extract Core Logic Modules
    - [ ] Write Failing Tests: Define tests for isolated logic components before extraction.
    - [ ] Implement: Move logic out of massive files into specific, logically grouped modules.
    - [ ] Verify Coverage: Run tests and ensure >80% coverage for the new logic modules.
- [ ] Task: Extract Data and Configuration Modules
    - [ ] Write Failing Tests: Define tests for isolated data structures.
    - [ ] Implement: Move data structures and basic validation into dedicated module files.
    - [ ] Verify Coverage: Run tests and ensure >80% coverage for the new data modules.
